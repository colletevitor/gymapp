...
import { Plus, Trash2, Edit2, GripVertical, Play, ChevronDown, ChevronUp, X, Check } from 'lucide-react'
...
export default function Templates() {
  ...
  const addExToForm = (exerciseId: string) => {
    if (form.exercises.find(e => e.exerciseId === exerciseId)) return
    setForm(f => ({
      ...f,
      exercises: [...f.exercises, { exerciseId, defaultSets: 3, defaultReps: 12, defaultWeight: 20 }]
    }))
  }

  const removeExFromForm = (idx: number) => {
    setForm(f => ({ ...f, exercises: f.exercises.filter((_, i) => i !== idx) }))
  }

  const updateExInForm = (idx: number, field: string, value: number) => {
    setForm(f => ({
      ...f,
  ...
  const handleStart = (templateId: string) => {
    if (activeSession) {
      if (!confirm('Tem um treino em andamento. Cancelar e iniciar novo?')) return
    }
      ...
      {isFormOpen && (
        <div className="card" style={{ marginBottom: 16, borderColor: 'var(--accent)' }}>
            ...
            return (
              <div key={i} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  ...
                  <button className="btn btn-danger btn-icon" style={{ padding: 4 }} onClick={() => removeExFromForm(i)}>
                    <X size={14} />
                  </button>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                  {[
                    { label: 'Séries', field: 'defaultSets', value: fe.defaultSets },
                    { label: 'Reps', field: 'defaultReps', value: fe.defaultReps },
                    { label: 'Kg', field: 'defaultWeight', value: fe.defaultWeight },
                    ...
                    <div key={field}>
                      ...
                      <input
                        type="number"
                        value={value}
                        onChange={e => updateExInForm(i, field, parseFloat(e.target.value) || 0)}
          ...
          <div style={{ marginTop: 8, marginBottom: 14 }}>
            ...
            <select
              value=""
              onChange={e => { if (e.target.value) addExToForm(e.target.value) }}
              style={{ marginBottom: 0 }}
            >
              <option value="">Selecione...</option>
              {exercises
                .filter(e => !form.exercises.find(fe => fe.exerciseId === e.id))
                .map(e => (
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button className="btn btn-ghost btn-sm" onClick={() => { setShowNew(false); setEditing(null) }}>
              <X size={14} /> Cancelar
            </button>
          </div>
        </div>
            ...
            {isOpen && (
              <div style={{ padding: '0 16px 14px', borderTop: '1px solid var(--border)' }}>
                {tpl.exercises.map((te, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      alignItems: 'center',
                      padding: '8px 0',
                      borderBottom: i < tpl.exercises.length - 1 ? '1px solid var(--border)' : 'none',
                    }}>
                      <span style={{ fontSize: 14 }}>{ex?.name || te.exerciseId}</span>
                      <span style={{ fontSize: 12, color: 'var(--text2)' }}>
                        {te.defaultSets}×{te.defaultReps} · {te.defaultWeight}kg
                      </span>
                    </div>
                  )
                })}
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
                  <button className="btn btn-primary btn-sm" onClick={() => handleStart(tpl.id)}>
                    <Play size={13} fill="currentColor" /> Iniciar
                  </button>
                  <button className="btn btn-ghost btn-sm" onClick={() => startEdit(tpl.id)}>
                    <Edit2 size={13} /> Editar
                  </button>
                  <button className="btn btn-danger btn-sm" onClick={() => { if (confirm(`Deletar "${tpl.name}"?`)) deleteTemplate(tpl.id) }}>
                    <Trash2 size={13} />
                  </button>
                </div>
              </div>
