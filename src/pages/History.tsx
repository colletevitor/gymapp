...
export default function History() {
  const { sessions, exercises, users, currentUserId, deleteSession } = useStore()
  ...
  const getVolume = (session: typeof sessions[0]) => {
    return session.exercises.reduce((total, ex) =>
      total + ex.sets.filter(s => s.done).reduce((t, s) => t + s.weight * s.reps, 0), 0
    )
  }
    ...
    return (
      <div>
        ...
        <div className="empty">
          ...
          <p style={{ fontSize: 14 }}>Complete seu primeiro treino para ver o histórico aqui</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
              ...
              <div style={{ flex: 1, textAlign: 'left' }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  {session.durationMinutes && (
                    <span className="badge badge-accent" style={{ fontSize: 11 }}>{session.durationMinutes} min</span>
                  )}
                  <span className="badge badge-blue" style={{ fontSize: 11 }}>{doneSets}/{totalSets} séries</span>
                  {vol > 0 && (
                    <span className="badge" style={{ fontSize: 11, background: 'var(--bg3)', color: 'var(--text2)' }}>
                      {vol.toLocaleString('pt-BR')} kg vol.
                    </span>
                  )}
                </div>
              </div>
              ...
              <div style={{ padding: '0 16px 14px', borderTop: '1px solid var(--border)' }}>
                {session.exercises.map((ex, i) => (
                  <div key={i} style={{ marginTop: 12 }}>
                    <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text2)', marginBottom: 6 }}>
                      ...
                      {ex.sets.map((set, j) => (
                        <span
                          ...
                          style={{
                            ...
                            padding: '4px 8px',
                            borderRadius: 6,
                            background: set.done ? '#c8f13515' : 'var(--bg3)',
                            color: set.done ? 'var(--accent)' : 'var(--text3)',
                            border: `1px solid ${set.done ? '#c8f13530' : 'var(--border)'}`,
                          }}
                        >
                ...
                {session.notes && (
                  <p style={{ fontSize: 13, color: 'var(--text2)', marginTop: 12, fontStyle: 'italic' }}>
                    "{session.notes}"
                  </p>
                )}
                <button
                  ...
                  style={{ marginTop: 14 }}
                  onClick={() => { if (confirm('Deletar esse treino?')) deleteSession(session.id) }}
                >
      ...
      {mySessions.length > 20 && !showAll && (
        <button className="btn btn-ghost btn-full" style={{ marginTop: 10 }} onClick={() => setShowAll(true)}>
          Ver todos ({mySessions.length})
        </button>
      )}
    </div>
  )
