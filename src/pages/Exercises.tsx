    ...
    <div>
      ...
      {(showAdd || editId) && (
        <div className="card" style={{ marginBottom: 16, borderColor: 'var(--accent)' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button className="btn btn-ghost btn-sm" onClick={() => { setShowAdd(false); setEditId(null) }}>
              <X size={14} /> Cancelar
            </button>
          </div>
        </div>
      ...
      <div className="chip-row" style={{ marginBottom: 16 }}>
        ...
        {MUSCLE_GROUPS.map(g => (
          ...
          <button key={g} className={`chip ${filterGroup === g ? 'active' : ''}`} onClick={() => setFilterGroup(g === filterGroup ? '' : g)}>
            {g}
          </button>
        ))}
      </div>
              ...
              {editId === ex.id ? null : (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button className="btn btn-ghost btn-icon" onClick={() => startEdit(ex)}>
                      <Edit2 size={14} />
                    </button>
                    <button className="btn btn-danger btn-icon" onClick={() => { if (confirm(`Deletar "${ex.name}"?`)) deleteExercise(ex.id) }}>
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
