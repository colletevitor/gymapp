...
export default function Home() {
  ...
  const mySessionsCount = sessions.filter(s => s.userId === currentUserId).length
  const lastSession = sessions.filter(s => s.userId === currentUserId)[0]
  ...
  const handleStart = (templateId: string) => {
    if (activeSession) {
      if (confirm('Já tem um treino em andamento. Cancelar o atual e começar novo?')) {
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <button
          ...
          style={{
            ...
            height: 44,
            borderRadius: '50%',
            background: (currentUser?.color || '#888') + '22',
            border: `2px solid ${currentUser?.color || '#888'}`,
            color: currentUser?.color || '#888',
      ...
      <div className="stat-grid">
        <div className="stat-card">
          <div className="stat-value">{mySessionsCount}</div>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <button className="btn btn-ghost btn-sm" onClick={() => navigate('/fichas')}>
            Gerenciar
          </button>
        </div>
        {templates.length === 0 ? (
          <div className="empty">
            ...
            <button className="btn btn-primary" onClick={() => navigate('/fichas')}>
              Criar ficha
            </button>
          </div>
        ) : (
